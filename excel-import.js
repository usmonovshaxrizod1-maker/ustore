      ...
      <div class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-3" onclick="activePopupModal=null; render();">
        <div class="bg-white rounded-3xl p-4 max-w-md w-full max-h-[94vh] overflow-y-auto space-y-3 shadow-2xl text-xs" onclick="event.stopPropagation()">
          ...
          <div class="grid grid-cols-2 gap-2">
            <button onclick="UstoreExcel.downloadTemplate()" ${state.busy?'disabled':''} class="bg-slate-900 text-white font-bold py-2.5 rounded-xl">📥 ${xl('Yangi shablon','Новый шаблон')}</button>
            <label class="bg-blue-600 text-white font-bold py-2.5 rounded-xl text-center cursor-pointer ${state.busy?'opacity-50':''}" >📤 ${xl('Excel tanlash','Выбрать Excel')}<input type="file" accept=".xlsx" class="hidden" onchange="UstoreExcel.handleFile(event)" ${state.busy?'disabled':''}></label>
          </div>
